...
(function() {
  var EPub, Q, cheerio, ejs, exec, fs, fsextra, path, removeDiacritics, request, uslug, uuid, _,
  ...
  EPub = (function() {
    function EPub(options, output) {
      ...
      if (this.options.version === 2) {
        ...
        this.options.docHeader = "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"no\"?>\n<!DOCTYPE html>\n<html xmlns=\"http://www.w3.org/1999/xhtml\" lang=\"" + self.options.lang + "\">";
      }
      ...
      this.options.content = _.map(this.options.content, function(content, index) {
        var $, allowedAttributes, allowedV2Tags, titleSlug;
        ...
        $("*").each(function(index, elem) {
          var attrs, child, k, that, v, _ref, _ref1;
          attrs = elem.attribs;
          that = this;
          if ((_ref = that.name) === "img" || _ref === "br" || _ref === "hr") {
            $(that).text("");
            if (that.name === "img") {
          ...
          for (k in attrs) {
            v = attrs[k];
            if (__indexOf.call(allowedAttributes, k) >= 0) {
              if (k === "type") {
                if (that.name !== "script") {
                  $(that).removeAttr(k);
                }
          ...
          if (self.options.version === 2) {
            if (_ref1 = that.name, __indexOf.call(allowedV2Tags, _ref1) >= 0) {
    ...
    EPub.prototype.generateTempFile = function() {
      var generateDefer, htmlTocPath, ncxTocPath, opfPath, self, _base;
      ...
      opfPath = self.options.customOpfTemplatePath || path.resolve(__dirname, "../template" + (self.options.version === 2 ? "/v2" : "") + "/content.opf");
      if (!fs.existsSync(opfPath)) {
        generateDefer.reject(new Error('Custom file to OPF template not found.'));
        return generateDefer.promise;
      }
      ncxTocPath = self.options.customNcxTocTemplatePath || path.resolve(__dirname, "../template" + (self.options.version === 2 ? "/v2" : "") + "/toc.ncx");
      if (!fs.existsSync(ncxTocPath)) {
        generateDefer.reject(new Error('Custom file the NCX toc template not found.'));
        return generateDefer.promise;
      }
      htmlTocPath = self.options.customHtmlTocTemplatePath || path.resolve(__dirname, "../template" + (self.options.version === 2 ? "/v2" : "") + "/toc.xhtml");

    ...
    EPub = class EPub {
        constructor(options, output) {
            ...
            this.options.content = _.map(this.options.content, function(content, index) {
                ...
                $("div").each(function(index, elem) {
                    ...
                    if (style) {
                        if (style.indexOf('background-image') >= 0) {
                            const urls = style.match(regexp);
                            if (urls) {
                                url = urls[0].replace(")", "");
                                const splitArray = url.split("/");
                                const fileName = splitArray[splitArray.length - 1].split(".");
                                id = fileName[0];
                                extension = fileName[1];
                                if (url.trim().endsWith(".jpg") || url.trim().endsWith(".jpeg")) {
                                    mediaType = "image/jpeg";
                                } else if (url.trim().endsWith(".png")) {
                                    mediaType = "image/png";
                                } else if (url.trim().endsWith(".svg")) {
                                    mediaType = "image/svg+xml";
                                } else if (url.trim().endsWith(".gif")) {
                                    mediaType = "image/gif";
                                } else {
                                    mediaType = "NOT_AN_IMAGE";
                                }
                                if (mediaType != "NOT_AN_IMAGE") {
                                    dir = content.dir;
                                    const found = self.options.images.find(element => element.url == url);
                                    if (!found) {
                                        self.options.images.push({ id, url, dir, mediaType, extension });
                                    }
                                    return $(elem).attr("style", style.replace(url, "images/" + id + "." + extension));
                                }
                            }
                        }
                    }
                ...
                self.options.images = [...self.options.images, ...options.externalImages];
                self.options.images = self.options.images.filter((image, index, self) =>
                    index === self.findIndex((t) => (
                        t.url === image.url && t.id === image.id
                    ))
                )
                content.data = $.xml();
                return content;
        ...
        render() {
            ...
            return this.generateTempFile().then(function() {
                ...
                return self.downloadAllFonts().fin(function() {
                    if (self.options.verbose) {
        ...
        generateTempFile() {
            ...
            _.each(this.options.content, function(content) {
                var data;
                data = `${self.options.docHeader}\n  <head>\n  <meta charset="UTF-8" />\n  <title>${entities.encodeXML(content.title.replace(/[^a-zA-Z ]/g, "") || content.filename.replace(/[^a-zA-Z ]/g, ""))}</title>\n  <link rel="stylesheet" type="text/css" href="style.css" />\n  </head>\n<body>`;
        ...
        downloadFont(options) { //{id, url, mediaType}
            var auxpath, downloadFontDefer, filename, requestAction, self, userAgent;
            self = this;
            userAgent = "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_9_2) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/34.0.1847.116 Safari/537.36";
            if (!options.url && typeof options !== "string") {
                return false;
            }
            downloadFontDefer = new Q.defer();
            filename = path.resolve(self.uuid, "./OEBPS/fonts/" + options.id);
            if (options.url.indexOf("file://") === 0) {
                auxpath = options.url.substr(7);
                fsextra.copySync(auxpath, filename);
                return downloadFontDefer.resolve(options);
            } else {
                if (options.url.indexOf("http") === 0) {
                    requestAction = request.get(options.url).set({
                        'User-Agent': userAgent
                    ...
                    requestAction = fs.createReadStream(path.resolve("fonts", options.url));
                    requestAction.pipe(fs.createWriteStream(filename));
                }
                requestAction.on('error', function(err) {
                    if (self.options.verbose) {
                    ...
                    fs.unlinkSync(filename);
                    return downloadFontDefer.reject(err);
                });
                requestAction.on('end', function() {
                    if (self.options.verbose) {
                        console.log("[Download Success]", options.url);
                ...
                return downloadFontDefer.promise;
            }
        }
        ...
        downloadAllFonts() {
            var deferArray, fontDefer, self;
            self = this;
            fontDefer = new Q.defer();
            if (!self.options.externalFonts.length) {
                console.log(self.options.externalFonts);
                fontDefer.resolve();
            } else {
                console.log(self.options.externalFonts);
                try {
                    if (self.options.externalFonts && self.options.externalFonts != null && self.options.externalFonts.length > 0) {
                        //fs.mkdirSync(path.resolve(this.uuid, "./OEBPS/fonts"));
                        deferArray = [];
                        _.each(self.options.externalFonts, function(font) {
                            return deferArray.push(self.downloadFont(font));
                        });
                        Q.all(deferArray).fin(function() {
                            return fontDefer.resolve();
                        });
                    } else {
                        return fontDefer.resolve();
                    }

...
(function() {
  ...
  EPub = (function() {
    ...
    EPub.prototype.generateTempFile = function() {
      ...
      opfPath = self.options.customOpfTemplatePath || path.resolve(__dirname, "../templates/epub" + self.options.version + "/content.opf");
      if (!fs.existsSync(opfPath)) {
        generateDefer.reject(new Error('Custom file to OPF template not found.'));
        return generateDefer.promise;
      }
      ncxTocPath = self.options.customNcxTocTemplatePath || path.resolve(__dirname, "../templates/epub" + self.options.version + "/toc.ncx");
      if (!fs.existsSync(ncxTocPath)) {
        generateDefer.reject(new Error('Custom file the NCX toc template not found.'));
        return generateDefer.promise;
      }
      htmlTocPath = self.options.customHtmlTocTemplatePath || path.resolve(__dirname, "../templates/epub" + self.options.version + "/toc.xhtml");

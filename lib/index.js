...
(function() {
  var EPub, Q, _, archiver, cheerio, ejs, entities, exec, fs, fsextra, mime, path, removeDiacritics, request, rimraf, uslug, uuid,
  ...
  EPub = (function() {
    ...
    EPub.prototype.generateTempFile = function() {
      ...
      _.each(this.options.content, function(content) {
        ...
        data += content.title && self.options.appendChapterTitles ? "<h1>" + (entities.encodeHTML(content.title)) + "</h1>" : "";
        data += content.title && content.author && content.author.length ? "<p class='epub-author'>" + (entities.encodeHTML(content.author.join(", "))) + "</p>" : "";

...
(function() {
  var EPub, Q, _, archiver, cheerio, ejs, entities, fs, fsextra, mime, mkdirp, os, path, removeDiacritics, request, rimraf, uslug, uuid,
    indexOf = [].indexOf;
  ...
  EPub = class EPub {
    ...
    makeCover() {
      var coverDefer, destPath, writeStream;
      ...
      if (this.options.cover) {
        ...
        if (this.options.cover.slice(0, 4) === "http") {
          writeStream = request.get(this.options.cover).timeout({
            response: this.options.httpSetting.reqTimeout,
            deadline: this.options.httpSetting.readTimeout
          }).retry(this.options.httpSetting.retryTimes).set({

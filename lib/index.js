...
(function() {
  var EPub, Q, _, archiver, cheerio, ejs, entities, fs, fsextra, got, mime, mkdirp, os, path, removeDiacritics, rimraf, uslug, uuid,
    indexOf = [].indexOf;
  ...
  EPub = class EPub {
    constructor(options, output) {
      ...
      if (!this.options.tempDir) {
        this.options.tempDir = path.resolve(os.tmpdir(), "epub-gen");
      }
      this.id = uuid();
    ...
    makeCover() {
      var coverDefer, destPath, inputStream;
      ...
      if (this.options.cover) {
        ...
        if (this.options.cover.slice(0, 4) === "http") {
          inputStream = got.stream(this.options.cover, {
            timeout: {
              request: this.options.httpSetting.timeout
            },
            retry: this.options.httpSetting.retryTimes,
            headers: {
              'User-Agent': this.options.httpSetting.userAgent
            }
          });
          inputStream.pipe(fs.createWriteStream(destPath));
        } else {
          inputStream = fs.createReadStream(this.options.cover);
          inputStream.pipe(fs.createWriteStream(destPath));
        }
    ...
    downloadImage(options) { //{id, url, mediaType}
      var auxpath, downloadImageDefer, filename, inputStream, self;
      self = this;

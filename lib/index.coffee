    ...
    @options.content = _.map @options.content, (content, index)->
      ...
      $($("*").get().reverse()).each (elemIndex, elem)->
        ...
        if self.options.version is 2
          if that.name in allowedXhtmlTags
          else
            console.log "Warning (content[" + index + "]):", that.name, "tag isn't allowed on XHTML/EPUB 2."
            child = $(that).html()
